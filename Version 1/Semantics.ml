...
let rec value_of_expr expr env =
  ...
  and
    ...
    match condvalue with
      | (ErrorValue _) as result -> result
      | (BooleanValue value) -> if value then (value_of_expr bthen env) else (value_of_expr belse env)
      | _ -> (ErrorValue TypeMismatchError)
  ...
  and
    ...
    let callvalue = (value_of_expr fexpr env) in
    match callvalue with
      | (FrozenValue (fexpr, fenv)) -> 
        (match fexpr with
          | (FunctionNode (fpar, fbody)) -> (value_of_expr fbody ((fpar, (FrozenValue (fexpr, env)))::fenv))
          | _ -> (ErrorValue TypeMismatchError))
      | (ErrorValue _) as result -> result
      | _ -> (ErrorValue TypeMismatchError)
    (* A compléter *)
  ...
  and
    ...
    (* Appel par valeur *)
    match (value_of_expr fexpr pexpr env) with
    | (FrozenValue (fexpr,fenv)) ->
    (match fexpr with 
    	| (FunctonNode (fpar,fbody)) ->
    		(match (value_of_expr pexpr env) with 
    		| (ErrorValue _) as result -> result
    		| result -> (value_of_expr fbosy ((fpar, result)::fenv))
    		| _ -> (ErrorValue TypeMismatchError))
    	| (ErrorValue _) as result -> result
    	| _ -> (ErrorValue TypeMismatchError))
    (* A compléter *)
  ...
  and
    ...
    value_of_expr bin ((ident,(FrozenValur ((LetrecNode (ident,bvalue,bvalue)),env)))::env)
    (* A compléter *)
