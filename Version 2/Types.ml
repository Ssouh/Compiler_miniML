...
ruleFunction par body env =
  let typepar = newVariable () in
    let typeres = (type_of_expr body ((par,typepar)::env)) in
      FunctionType(typepar,typeres)
and 
(* ...............A COMPLETER .......................................*)
...
ruleWrite left right env =
  match (type_of_expr left env) with
    | (ErrorType| UnknownType) -> ErrorType
    | (ReferenceType tleft) -> 
                      (match (type_of_expr right env) with
                        | (ErrorType| UnknownType) -> ErrorType
                        | tright ->  (let utl,url = unify tleft tright in
...
ruleSequence left right env =
    match type_of_expr left env with
      | UnitType -> let tright = (type_of_expr right env) in
                      tright
      | _ -> ErrorType
